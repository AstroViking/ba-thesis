...
The implementation of the algorithm in TensorFlow proposed in this paper was also inspired by KDEpy[@KDEpy] and uses Brent's method[@brent1971algorithm] to find roots, implemented in TensorFlow in the python package `tf_quant_finance`.

However while in KDEpy the algorithm is only used to calculate the optimal bandwidth, in the new implementation the algorithm can also be used directly to infer the kernel density estimation.
