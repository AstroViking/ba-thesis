...
The optimal bandwidth is often defined as the one that minimizes the mean integrated square error ($MISE$), which is given by the expectation value $\mathbb{E}_f$ of the integrated square error between the Kernel Density Estimation $\widehat{f}_{h,norm}(x)$ and the true probability density function $f(x)$.
...
## Using specialized kernel functions and their series expansion

Lastly there is an interesting approach described by Hofmeyr[@hofmeyrFastExactEvaluation2019] that uses special kernels of the form $K(x) := poly(|x|) \cdot exp(−|x|)$ where $poly(|x|)$ denotes a polynomial of finite degree.

Given the kernel with a polynom of order $\alpha$

\begin{equation}
K(x) := \sum_{j=0}^{\alpha} |x|^j \cdot e^{−|x|}
(\#eq:polyexpkernel)
\end{equation}
...
Using this recursion one can then calculate the Kernel Density Estimation with a single single forward
and a single backward pass over the ordered set of all $x_{\tilde{n}}$ leading to a computational complexity of $\mathcal{O}((\alpha+1)(n+m))$ where $\alpha$ is the order of the polynom, $n$ is the number of sample points and $m$ is the number of evaluation points.