...
First, the exact kernel density estimation implementation is compared against binned and FFT implementations run on a Macbook Pro 2013 Retina using the CPU.
...
For the binned, FFT and ISJ algorithms 256 bins each were used.
...
For this randomly sampled data for each test distribution in the range of 100 to 1000 is used. The number of samples is restricted because larger datasets would require exponentially longer runtimes for the exact kernel density estimation.
